    ...
    public static void symbol(ASTNode parent){
        if(parent instanceof TNode){
            if(((TNode)parent).token.equals("printf")) {
                ...
                //     boolean flag=false;
                //     for(Element element:symbol.currentSTTNode.que.que){
                //         if(element.name.equals("return")){
                //             flag=true;
                //         }
                //     }
                //     if(!flag) symbol.writeFile(symbol.errorFile, ((TNode)parent).lineNumber+" g\n");
                // }
                if(parent.parent.parent.name.equals("<FuncDef>")||parent.parent.parent.name.equals("<MainFuncDef>")) {
                    ...
                    // else if(symbol.inMainFlag){
                    //     if(!symbol.getASTNodeContent(parent.parent, new int[] {parent.parent.children.size()-2,0,0}).equals("return"))
                    // }
                    // else{
                    //     if(symbol.getASTNodeContent(parent.parent, new int[] {parent.parent.children.size()-2,0,0}).equals("return")){
                    //         symbol.writeFile(symbol.errorFile, ((TNode)parent).lineNumber+" f\n");
