...
public class Module {
    ...
    public static GlobalValue createGlobalValue(VarType type,String name,ASTNode AddExp) {
            ...
            newGlobalValue.createInitVal(type,AddExp);
        ...
        public static void orderAST(ASTNode parent){
            //visit
            if(parent.name.equals("<MainFuncDef>")){
                globalFlag=false;
                createFunction(new ReturnType("int"),"main", null);
            }
            else if(parent.name.equals("<Block>")){
                functions.get(functions.size()-1).createBasicBlock();
            }
                ...
                if(globalFlag){
                    ...
                    for(int i=0;i<num;i++){
                        if(symbol.getASTNode(parent,new int[] {2*i+2}).children.size()==3){//变量
                            String name=symbol.getASTNodeContent(parent,new int[] {2,0,0});
                ...
                createFunction(retType,funcName, paraTypes);
            }
